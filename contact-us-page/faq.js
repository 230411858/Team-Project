...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Select elements where content will be updated - GPT assistance was buggy
    ...
    if (product && faqs[product]) {
      // Update product title by formatting the product name
      titleElement.textContent = product.replace(/([A-Z])/g, " $1").trim();
  
      // Update product image with corresponding file
      imageElement.src = `images/products/mice/${product}(1).png`;
      imageElement.alt = `${product} Image`;
  
      // Generate FAQ list dynamically
    ...
    //  click event listeners to each FAQ question
    document.querySelectorAll(".faq-question").forEach((button) => {
      button.addEventListener("click", () => {
        const answer = button.nextElementSibling;
  
        // Toggle the visibility of the clicked FAQ answer - https://stackoverflow.com/questions/28343333/how-to-create-a-hide-and-show-faq 
